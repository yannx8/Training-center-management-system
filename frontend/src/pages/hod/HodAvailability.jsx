  ...
  return (
    <div className="space-y-4">
      ...
      <div className={cn('card', 'p-4', 'space-y-3')}>
        <div className={cn('flex', 'flex-wrap', 'gap-3', 'items-end')}>
        ...
        {isLocked && weekId && (
          <p className={cn('text-sm', 'text-amber-700', 'bg-amber-50', 'border', 'border-amber-200', 'rounded-xl', 'px-3', 'py-2')}>
            {t('availability.locked','🔒 Locked — trainers cannot edit')}
          </p>
        )}

        {!weekId && (
          <p className={cn('text-sm', 'text-gray-400')}>{t('availability.selectWeekFirst','Select a published week to see availability status.')}</p>
        )}
        {weekId && publishedWeeks.length === 0 && (
          <p className={cn('text-sm', 'text-gray-400')}>{t('availability.noPublishedWeeks','No published weeks yet.')}</p>
        )}
      </div>
      ...
      {weekId && !loadingStatus && status && (
        <>
          {allDone && (
            <div className={cn('card', 'p-4', 'bg-green-50', 'border', 'border-green-200', 'text-green-700', 'text-sm', 'flex', 'items-center', 'gap-2')}>
          ...
          {submitted.length > 0 && (
            <div className={cn('card', 'overflow-hidden')}>
              <div className={cn('px-4', 'py-3', 'bg-green-50', 'border-b', 'border-green-100', 'flex', 'items-center', 'gap-2')}>
                <CheckCircle size={15} className="text-green-600"/>
                <h3 className={cn('font-semibold', 'text-green-800', 'text-sm')}>{t('availability.submitted','Submitted')} ({submitted.length})</h3>
              </div>
              <div className={cn('divide-y', 'divide-gray-50')}>
                {submitted.map(tr => {
                  ...
                  return (
                    <div key={tr.id}>
                      <button
                        className={cn('w-full', 'px-4', 'py-3', 'flex', 'items-center', 'justify-between', 'hover:bg-gray-50', 'transition-colors')}
                        onClick={() => setExpanded(e => ({ ...e, [key]: !e[key] }))}
                      >
                        <div className={cn('flex', 'items-center', 'gap-3', 'min-w-0')}>
                          <div className={cn('w-8', 'h-8', 'rounded-lg', 'bg-green-100', 'flex', 'items-center', 'justify-center', 'flex-shrink-0', 'text-green-700', 'text-xs', 'font-bold')}>
                            {tr.user?.fullName?.charAt(0).toUpperCase()}
                          </div>
                          <div className={cn('text-left', 'min-w-0')}>
                            <p className={cn('text-sm', 'font-medium', 'text-gray-900', 'truncate')}>{tr.user?.fullName}</p>
                            <p className={cn('text-xs', 'text-green-600')}>{t('availability.slots','{{count}} slot(s)', { count: tr.slotCount })}</p>
                          </div>
                        </div>
                      ...
                      {expanded[key] && tr.slots?.length > 0 && (
                        <div className={cn('px-4', 'pb-3', 'flex', 'flex-wrap', 'gap-1')}>
                          {tr.slots.map((s,i) => (
                            <span key={i} className={cn('text-xs', 'bg-green-50', 'border', 'border-green-200', 'text-green-700', 'rounded-lg', 'px-2', 'py-1')}>
          ...
          {notSubmitted.length > 0 && (
            <div className={cn('card', 'overflow-hidden')}>
              <div className={cn('px-4', 'py-3', 'bg-red-50', 'border-b', 'border-red-100', 'flex', 'items-center', 'gap-2')}>
                <XCircle size={15} className="text-red-500"/>
                <h3 className={cn('font-semibold', 'text-red-700', 'text-sm')}>{t('availability.notSubmitted','Not Submitted')} ({notSubmitted.length})</h3>
              </div>
              <div className={cn('divide-y', 'divide-gray-50')}>
                {notSubmitted.map(tr => (
                  <div key={tr.id} className={cn('px-4', 'py-3', 'flex', 'items-center', 'gap-3')}>
                    <div className={cn('w-8', 'h-8', 'rounded-lg', 'bg-red-50', 'flex', 'items-center', 'justify-center', 'flex-shrink-0', 'text-red-400', 'text-xs', 'font-bold')}>
                      {tr.user?.fullName?.charAt(0).toUpperCase()}
                    </div>
                    <p className={cn('text-sm', 'text-gray-700')}>{tr.user?.fullName}</p>
                  </div>
                ))}
              </div>
            </div>
