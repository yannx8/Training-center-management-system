...
export default function Certifications() {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader title={t('certifications.title','Certifications')} subtitle={t('certifications.subtitle','Standalone certifications')}>
        <button className="btn-primary" onClick={() => { setForm(EMPTY); setEditing(null); setModal(true); }}><Plus size={16}/> {t('certifications.addCertification','Add Certification')}</button>
      </SectionHeader>

      {certs.length === 0 && <div className="card p-10 text-center text-gray-400">{t('certifications.noCerts','No certifications yet.')}</div>}
          ...
          return (
            <div key={c.id} className="card px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{c.code} · {c.durationHours}h · {c.capacity ? `${c._count?.enrollments||0}/${c.capacity}` : `${c._count?.enrollments||0}`} {t('certifications.enrolments','enrolled')}</p>
              </div>
              {trainer
              ...
              <button className="btn-ghost btn-sm btn-icon text-blue-500" title={t('certifications.assignTrainer','Assign Trainer')}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? t('certifications.editCertification','Edit Certification') : t('certifications.addCertification','Add Certification')}
        ...
        <div className="space-y-4">
          <div><label className="label">{t('certifications.certName','Name')}</label><input className="input" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div>
          <div><label className="label">{t('certifications.certCode','Code')}</label><input className="input" value={form.code} onChange={e=>setForm(p=>({...p,code:e.target.value}))}/></div>
          <div><label className="label">{t('common.description','Description')}</label><textarea rows={2} className="input" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))}/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">{t('certifications.durationHours','Duration (hours)')}</label><input type="number" min={1} className="input" value={form.durationHours} onChange={e=>setForm(p=>({...p,durationHours:+e.target.value}))}/></div>
            <div><label className="label">{t('certifications.maxStudents','Max Students')}</label><input type="number" min={1} className="input" placeholder={t('common.noLimit','No limit')} value={form.capacity} onChange={e=>setForm(p=>({...p,capacity:e.target.value}))}/></div>
          </div>
      ...
      <Modal open={!!assignModal} onClose={() => setAssignModal(null)} title={t('certifications.assignTrainer','Assign Trainer')}
        footer={<><button className="btn-secondary" onClick={() => setAssignModal(null)}>{t('common.cancel','Cancel')}</button><button className="btn-primary" onClick={handleAssign}>{t('common.assign','Assign')}</button></>}>
        <div>
      ...
      <ConfirmModal open={!!deleteId} onClose={() => setDeleteId(null)}
        ...
        title={t('certifications.deleteCertification','Delete Certification')} message={t('certifications.deleteConfirm','All enrollments and grades will also be removed.')}/>
    </div>
  );
}