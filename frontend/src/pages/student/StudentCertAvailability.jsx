...
export default function StudentCertAvailability() {
    ...
    studentApi.getPublishedWeeks({ certificationId: certId })
      .then(async r => {
        ...
        if (latest) {
          ...
          setSlots((avail.data || []).map(a => ({ dayOfWeek: a.dayOfWeek, timeStart: a.timeStart.slice(0,5), timeEnd: a.timeEnd.slice(0,5) })));
        }
  ...
  function isSelected(day, time) { return slots.some(s => s.dayOfWeek === day && s.timeStart === time); }

  function toggleSlot(day, time) {
    setMsg('');
    const exists = slots.find(s => s.dayOfWeek === day && s.timeStart === time);
    if (exists) { setSlots(p => p.filter(x => !(x.dayOfWeek === day && x.timeStart === time))); }
    else { const idx = TIME_SLOTS.indexOf(time); const end = TIME_SLOTS[idx+1]||'19:00'; setSlots(p => [...p, { dayOfWeek: day, timeStart: time, timeEnd: end }]); }
  }
  ...
  async function handleSave() {
    ...
    } catch (e) { setError(e.response?.data?.message || t('common.failedSave','Failed to save')); }
  ...
  return (
    <div className="space-y-4">
      ...
      {!certId && certs.length > 1 && (
        <div className="card p-8 text-center text-gray-400"><p>{t('availability.chooseCert','Select a certification above.')}</p></div>
      )}
      ...
      {certId && !weekLoading && activeWeek && (
        <>
          <div className="card px-4 py-3">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Week {activeWeek.weekNumber} · {new Date(activeWeek.startDate).toLocaleDateString()} – {new Date(activeWeek.endDate).toLocaleDateString()}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button className="btn-secondary" onClick={() => setSlots([])}><Trash2 size={15}/> {t('availability.clearAll','Clear')}</button>
          </div>
          ...
          <div className="card overflow-hidden">
            ...
            <div className="px-4 py-2 border-b border-gray-100">
              <p className="text-xs text-gray-600"><span className="font-bold text-gray-900">{slots.length}</span> {t('availability.slotsSelected','slot(s) selected')}</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse" style={{ minWidth: '520px' }}>
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 w-28 sticky left-0 bg-gray-50 z-10">{t('time.timeSlot','Time')}</th>
                    {DAYS_EN.map(d => <th key={d} className="px-1 py-2.5 text-center text-xs font-semibold text-gray-600 min-w-[72px]">{t(`days.${d}`,d)}</th>)}
                  </tr>
                </thead>
                <tbody>
                        ...
                        return (
                          <td key={day} className="px-1 py-1 text-center">
                            <button onClick={() => toggleSlot(day,time)}
                              className={`w-full h-8 rounded-lg text-xs font-semibold transition-all border ${sel?'bg-violet-500 text-white border-violet-600 shadow-sm hover:bg-violet-600':'bg-white text-gray-300 border-gray-200 hover:bg-violet-50 hover:text-violet-500 hover:border-violet-300'}`}>
                              {sel?'✓':''}
                            </button>
                          </td>
                        );
