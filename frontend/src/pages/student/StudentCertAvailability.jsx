...
export default function StudentCertAvailability() {
  ...
  // Load cert enrollments on mount
  useEffect(() => {
  ...
  // When certId changes, load cert-specific published weeks for that cert
  useEffect(() => {
    if (!certId) { setWeeks([]); setWeekId(''); setSlots([]); return; }
    setWeekId('');
    setSlots([]);
  ...
  // When both certId and weekId are set, load existing availability
  useEffect(() => {
          ...
          <div className="card p-4 space-y-3">
            ...
            <div>
              ...
              <select className="select" value={certId} onChange={e => { setCertId(e.target.value); setMsg(''); }}>
                ...
                {certs.map(c => (
                  <option key={c.id} value={c.certificationId}>{c.certification?.name}</option>
                ))}
              </select>
            </div>

            {/* Week selector — only shows cert-specific weeks */}
            {certId && (
              <div>
                ...
                {weeks.length === 0 ? (
                  <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                    <Info size={14}/> No published weeks for this certification yet. Wait for your trainer to publish a week.
                  </div>
                ) : (
            ...
            {!canShowGrid && certId && weeks.length > 0 && (
              <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 rounded-xl px-3 py-2">
                <Info size={14}/> {t('availability.selectBothFirst', 'Please select a week.')}
              </div>
            )}
