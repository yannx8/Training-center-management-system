...
function StepDot({ n, active, done, label }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${done ? 'bg-primary-600 border-primary-600 text-white' :
...
export default function RegisterStudent() {
  ...
  useEffect(() => {
    if (form.programId) {
      const prog = programs.find(p => p.id === Number(form.programId));
  ...
  useEffect(() => {
    setParents(prev => prev.map(p => {
      if (p.firstName && p.lastName) {
        const email = `${p.firstName.toLowerCase().trim()}.${p.lastName.toLowerCase().trim()}`.replace(/\s+/g, '') + '@pylvia.com';
        return { ...p, email };
      }
      return { ...p, email: '' }; // keep email empty if names are missing
    }));
  ...
  async function handleSubmit() {
    ...
    try {
      // Validate all parent emails before submitting
      for (const p of parents) {
        if (!p.email) continue;
        if (!validateEmail(p.email)) {
          return setError(`Invalid email format for parent ${p.firstName} ${p.lastName}: ${p.email}`);
        }
      }
      ...
      const payload = {
        ...
        parents: parents.filter(p => p.firstName && p.lastName).map(p => ({
          ...
          <div className="card p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className="input" placeholder={t('secretary.lastNamePlaceholder', 'Enter last name')} value={form.lastName} onChange={e => setF('lastName', e.target.value)} required />
              </div>
              <div>
                <label className="label">{t('secretary.email', 'Email (Auto-generated)')}</label>
                <input type="email" className="input bg-gray-50 text-gray-400 cursor-not-allowed" value={form.email} readOnly disabled />
              </div>
              <div>
            ...
            {parents.map((p, i) => (
              <div key={i} className="border border-gray-200 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="label">{t('secretary.parentEmail', 'Email (Auto-generated)')}</label>
                    <input type="email" className="input bg-gray-50 text-gray-400 cursor-not-allowed" placeholder={t('secretary.parentEmailPlaceholder', 'parent@example.com')} value={p.email} readOnly disabled />
                  </div>
                  <div>
