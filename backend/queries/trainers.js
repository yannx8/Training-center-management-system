...
// Submit availability — unique per trainer+week+day+timeslot
...
function getAvailabilityLock(hodUserId, academicWeekId = null) {
    const weekFilter = academicWeekId ? 'AND academic_week_id = $2' : '';
    const params = academicWeekId ? [hodUserId, academicWeekId] : [hodUserId];
    const sql = `
        SELECT is_locked, academic_week_id
        FROM availability_locks
        WHERE hod_user_id = $1 ${weekFilter}
        LIMIT 1
    `;
    return [sql, params];
}
