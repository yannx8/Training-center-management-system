...
function createAcademicWeek(academicYearId, weekNumber, label, startDate, endDate, createdBy, departmentId) {
    return [
        `INSERT INTO academic_weeks
         ...
         RETURNING *`, [academicYearId || null, weekNumber, label, startDate, endDate, createdBy, departmentId]
    ];
}
...
function getAcademicWeeksByDepartment(departmentId) {
    return [
        `SELECT aw.*, COALESCE(ay.name,'') AS year_name
         ...
         ORDER BY aw.start_date DESC`, [departmentId]
    ];
}
...
function createCertWeek(certificationId, weekNumber, label, startDate, endDate, createdBy) {
    return [
        `INSERT INTO academic_weeks
         ...
         RETURNING *`, [weekNumber, label, startDate, endDate, createdBy, certificationId]
    ];
}
...
function getCertWeeksByCert(certificationId) {
    return [
        `SELECT aw.*, cert.name AS certification_name, cert.code AS certification_code
         ...
         ORDER BY aw.created_at DESC`, [certificationId]
    ];
}

// All cert weeks ever created for a cert — for the student's history page
function getAllCertWeeksForStudent(studentId) {
    return [
        `SELECT DISTINCT aw.id AS week_id, aw.label AS week_label,
                aw.start_date, aw.end_date, aw.status,
                cert.id AS certification_id, cert.name AS certification_name
         FROM enrollments e
         JOIN certifications cert ON e.certification_id = cert.id
         JOIN academic_weeks aw ON aw.certification_id = cert.id AND aw.week_type = 'certification'
         WHERE e.student_id = $1
         ORDER BY cert.id, aw.start_date ASC`, [studentId]
    ];
}

// Latest published cert week per cert — what students and trainer see for availability
    ...
    return [
        `SELECT aw.*, cert.name AS certification_name, cert.code AS certification_code
         ...
         WHERE aw.certification_id = $1
           ...
           AND aw.status = 'published'
         ORDER BY aw.created_at DESC
         LIMIT 1`, [certificationId]
    ];
}

// For each cert a student is enrolled in, return the latest published cert week
    ...
    return [
        `SELECT DISTINCT ON (cert.id)
         ...
         JOIN academic_weeks aw
            ...
            AND aw.status = 'published'
         WHERE e.student_id = $1
         ORDER BY cert.id, aw.created_at DESC`, [studentId]
    ];
}
...
// ─── ACADEMIC TIMETABLE (HOD generates, read-only) ────────────────────────────
...
function checkTrainerConflict(trainerId, dayOfWeek, timeStart, timeEnd, weekId) {
    return [
        `SELECT id FROM timetable_slots
         ...
         WHERE trainer_id=$1 AND day_of_week=$2 AND academic_week_id=$5
           AND time_start < $4::time AND time_end > $3::time`, [trainerId, dayOfWeek, timeStart, timeEnd, weekId]
    ];
}
...
function checkRoomConflict(roomId, dayOfWeek, timeStart, timeEnd, weekId) {
    return [
        `SELECT id FROM timetable_slots
         ...
         SELECT id FROM cert_timetable_slots
         WHERE room_id=$1 AND day_of_week=$2 AND academic_week_id=$5
           AND time_start < $4::time AND time_end > $3::time`, [roomId, dayOfWeek, timeStart, timeEnd, weekId]
    ];
}
...
function createTimetableSlot(timetableId, dayOfWeek, timeStart, timeEnd, roomId, trainerId, courseId, academicWeekId) {
    return [
        `INSERT INTO timetable_slots
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [timetableId, dayOfWeek, timeStart, timeEnd, roomId, trainerId, courseId, academicWeekId]
    ];
}

function getTimetableSlotsByWeek(weekId) {
    return [
        `SELECT ts.id, ts.day_of_week, ts.time_start, ts.time_end,
         ...
         WHERE ts.academic_week_id = $1
         ORDER BY
             CASE ts.day_of_week
                 WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3
                 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6
             END, ts.time_start`, [weekId]
    ];
}
        ...
        `SELECT ts.day_of_week, ts.time_start, ts.time_end,
         ...
         ORDER BY
             CASE ts.day_of_week
                 ...
                 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6
             END, ts.time_start`,
        weekId ? [studentId, weekId] : [studentId]
    ];
}
...
function getAllStudentWeeks(studentId) {
    return [
        `SELECT DISTINCT aw.id, aw.label, aw.start_date, aw.end_date, aw.status
         ...
         ORDER BY aw.start_date DESC`, [studentId]
    ];
}
...
function createCertTimetableSlot(certificationId, trainerId, academicWeekId, dayOfWeek, timeStart, timeEnd, roomId) {
    return [
        `INSERT INTO cert_timetable_slots
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [certificationId, trainerId, academicWeekId, dayOfWeek, timeStart, timeEnd, roomId || null]
    ];
}

function deleteCertSlotsForWeek(certificationId, academicWeekId) {
    return [
        `DELETE FROM cert_timetable_slots WHERE certification_id=$1 AND academic_week_id=$2`, [certificationId, academicWeekId]
    ];
}

// All generated cert timetables for a trainer — summary list
function getCertTimetablesByTrainer(trainerId) {
    return [
        `SELECT DISTINCT
         ...
         ORDER BY aw.start_date DESC`, [trainerId]
    ];
}
        ...
        `SELECT cts.id, cts.day_of_week, cts.time_start, cts.time_end,
         ...
         ORDER BY
             CASE cts.day_of_week
                 WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3
                 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6
             END, cts.time_start`, [certificationId, weekId]
    ];
}

// All cert timetable slots for student — history view (all weeks, all certs)
...
function getStudentCertTimetableWeeks(studentId) {
    return [
        `SELECT DISTINCT aw.id, aw.label, aw.start_date, aw.end_date,
         ...
         JOIN certifications cert ON cert.id = cts.certification_id
         ORDER BY aw.start_date DESC`, [studentId]
    ];
}
...
function getAllTrainerWeeks(trainerId) {
    return [
        `SELECT DISTINCT aw.id, aw.label, aw.start_date, aw.end_date, aw.status, aw.week_type
         FROM academic_weeks aw
         WHERE aw.id IN (
...
function submitStudentAvailability(studentId, academicWeekId, dayOfWeek, timeStart, timeEnd) {
    return [
        `INSERT INTO student_availability (student_id, academic_week_id, day_of_week, time_start, time_end)
         ...
         RETURNING *`, [studentId, academicWeekId, dayOfWeek, timeStart, timeEnd]
    ];
}
...
function createAnnouncement(title, body, targetRole, departmentId, createdBy) {
    return [
        `INSERT INTO announcements (title, body, target_role, department_id, created_by)
         VALUES ($1,$2,$3,$4,$5) RETURNING *`, [title, body, targetRole || null, departmentId || null, createdBy]
    ];
}

function getAnnouncementsByDepartment(departmentId, role) {
    const roleFilter = role ? `AND (target_role IS NULL OR target_role = $2)` : '';
    return [
        `SELECT a.*, u.full_name AS author_name
         FROM announcements a
         LEFT JOIN users u ON a.created_by = u.id
         WHERE (a.department_id = $1 OR a.department_id IS NULL) ${roleFilter}
...
function getAnnouncementsForTrainer(trainerId) {
    return [
        `SELECT DISTINCT a.*, u.full_name AS author_name, d.name AS department_name
         ...
         AND (a.target_role IS NULL OR a.target_role = 'trainer')
         ORDER BY a.created_at DESC`, [trainerId]
    ];
}
...
function getAnnouncementsForStudent(studentId) {
    return [
        `SELECT a.*, u.full_name AS author_name, d.name AS department_name
         ...
         AND (a.target_role IS NULL OR a.target_role = 'student')
...
function getAnnouncementsForParent(userId) {
    return [
        `SELECT DISTINCT a.*, u.full_name AS author_name, d.name AS department_name
         ...
         AND (a.target_role IS NULL OR a.target_role = 'parent')
...
function getStudentGradesByPeriod(studentId, periodId) {
    return [
        `SELECT g.*, c.name AS course_name, c.code AS course_code,
         ...
         WHERE g.student_id = $1
           AND ($2::int IS NULL OR g.academic_year_id = $2)
         ORDER BY g.submitted_at DESC`, [studentId, periodId || null]
    ];
}
...
function getStudentMarkComplaints(studentId) {
    return [
        `SELECT mc.id, mc.subject, mc.status, mc.created_at,
                mc.description, mc.trainer_response,
                c.name AS course_name, c.code AS course_code,
                cert.name AS certification_name,
                u.full_name AS trainer_name
         ...
         ORDER BY mc.created_at DESC`, [studentId]
    ];
}

function checkExistingMarkComplaint(studentId, courseId, certificationId) {
    if (courseId) {
        return [`SELECT id FROM mark_complaints WHERE student_id=$1 AND course_id=$2`, [studentId, courseId]];
    }
    return [`SELECT id FROM mark_complaints WHERE student_id=$1 AND certification_id=$2`, [studentId, certificationId]];
}
...
module.exports = {
    ...
    publishCertWeek,
    // Academic timetable (HOD)
    createTimetable,
    getTimetablesByDepartment,
    checkTrainerConflict,
    checkRoomConflict,
    createTimetableSlot,
    getTimetableSlotsByWeek,
