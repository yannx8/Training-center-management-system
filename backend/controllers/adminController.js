...
const getUsers = asyncHandler(async (req, res) => {
  ...
  const users = await prisma.user.findMany({
    ...
    include:{ userRoles:{ include:{ role:true } } }, orderBy:{ fullName:"asc" },
  });
...
const updateUserHandler = asyncHandler(async (req, res) => {
  ...
  const user = await prisma.user.update({ where:{ id:Number(id) }, data:{ fullName, email:email?.toLowerCase().trim(), phone, department, status } });
  if (roles !== undefined) {
...
module.exports = {
  ...
  getSessionsForProgramHandler, createSessionHandler, getSemestersHandler,
