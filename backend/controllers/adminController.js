...
// Adds: capacity management, programs grouped by dept, trainer assignment fixed
...
const createUserHandler = asyncHandler(async (req, res) => {
  ...
  if (roleName === "trainer") {
    await prisma.trainer.upsert({ where: { userId: user.id }, update: {}, create: { userId: user.id } });
  }
  if (roleName === "hod" && department) {
    await prisma.department.updateMany({ where: { name: department }, data: { hodUserId: user.id, hodName: fullName } });
  }
...
const updateUserHandler = asyncHandler(async (req, res) => {
  ...
  const user = await prisma.user.update({
    where: { id: Number(id) },
    data: { fullName, email: email?.toLowerCase().trim(), phone, department, status },
  });
  if (roles !== undefined) {
...
const getDepartmentsHandler = asyncHandler(async (req, res) => {
  const depts = await prisma.department.findMany({
    include: { hod: { select: { fullName: true, email: true } }, _count: { select: { programs: true } } },
    orderBy: { name: "asc" },
  });
...
// ── PROGRAMS (grouped by department in response) ──────────────────
...
const createProgramHandler = asyncHandler(async (req, res) => {
  ...
  const program = await prisma.program.create({
    data: { name, code, departmentId: departmentId ? Number(departmentId) : null, durationYears: Number(durationYears) || 3, status: status || "active", capacity: capacity ? Number(capacity) : null },
  });
...
const updateProgramHandler = asyncHandler(async (req, res) => {
  ...
  const program = await prisma.program.update({
    where: { id: Number(id) },
    data: { name, code, departmentId: departmentId ? Number(departmentId) : null, durationYears: Number(durationYears), status, capacity: capacity ? Number(capacity) : null },
  });
...
const getProgramCoursesHandler = asyncHandler(async (req, res) => {
  ...
  const sessions = await prisma.session.findMany({
    ...
    include: {
      academicLevel: true, semester: true, academicYear: true,
      courses: {
        include: { trainerCourses: { include: { trainer: { include: { user: true } } } } },
        orderBy: { name: "asc" },
      },
    },
...
const createCourseHandler = asyncHandler(async (req, res) => {
  ...
  const course = await prisma.course.create({
    data: { name, code, credits: Number(credits) || 3, hoursPerWeek: Number(hoursPerWeek) || 2, sessionId: Number(sessionId) },
  });
...
// FIX: Trainer assignment — properly upsert, not just delete+create
...
const getCertificationsHandler = asyncHandler(async (req, res) => {
  const certs = await prisma.certification.findMany({
    include: {
      trainerCourses: { include: { trainer: { include: { user: true } } } },
      _count: { select: { enrollments: true } },
    },
    orderBy: { name: "asc" },
  });
...
const createCertificationHandler = asyncHandler(async (req, res) => {
  ...
  const cert = await prisma.certification.create({
    data: { name, code, description, durationHours: Number(durationHours) || 40, status: status || "active", capacity: capacity ? Number(capacity) : null },
  });
...
const updateCertificationHandler = asyncHandler(async (req, res) => {
  ...
  const cert = await prisma.certification.update({
    where: { id: Number(req.params.id) },
    data: { name, code, description, durationHours: Number(durationHours), status, capacity: capacity ? Number(capacity) : null },
  });
...
// FIX: Cert trainer assignment — properly remove and reassign
const assignTrainerToCertHandler = asyncHandler(async (req, res) => {
  ...
  if (trainerId) {
    await prisma.trainerCourse.create({ data: { trainerId: Number(trainerId), certificationId: certId } });
  }
  return res.json({ success: true, data: { assigned: !!trainerId } });
});
...
const createRoomHandler = asyncHandler(async (req, res) => {
  ...
  // FIX: Default code = name if not provided
  const finalCode = code || name.replace(/\s+/g, "-").toUpperCase();
  const room = await prisma.room.create({
    data: { name, code: finalCode, building, capacity: Number(capacity) || 30, roomType: roomType || "Classroom", status: status || "available" },
  });
...
const createAcademicYearHandler = asyncHandler(async (req, res) => {
  ...
  const year = await prisma.academicYear.create({
    data: { name, startDate: new Date(startDate), endDate: new Date(endDate), isActive: !!isActive, programId: programId ? Number(programId) : null, certificationId: certificationId ? Number(certificationId) : null },
  });
...
const createSessionHandler = asyncHandler(async (req, res) => {
  ...
  const session = await prisma.session.create({
    data: { programId: Number(programId), academicYearId: academicYearId ? Number(academicYearId) : null, academicLevelId: academicLevelId ? Number(academicLevelId) : null, semesterId: semesterId ? Number(semesterId) : null },
  });
...
module.exports = {
  ...
  getProgramCoursesHandler, createCourseHandler, updateCourseHandler, deleteCourseHandler,
  assignTrainerHandler, getCertificationsHandler, createCertificationHandler, updateCertificationHandler,
  deleteCertificationHandler, assignTrainerToCertHandler, getRoomsHandler, createRoomHandler,
  updateRoomHandler, deleteRoomHandler, getAcademicYearsHandler, createAcademicYearHandler,
  getComplaintsHandler, updateComplaintHandler, getSessionsForProgramHandler, createSessionHandler,
  getAcademicLevelsHandler, getSemestersHandler, getTrainersByDeptHandler,
};
